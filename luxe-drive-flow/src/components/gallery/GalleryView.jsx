...
export default function GalleryView() {
  ...
  const openViewer = useCallback((index) => setSelectedIndex(index), []);
    ...
    <section id="top" className="relative pt-28 md:pt-32 pb-12 md:pb-16">
      <div className="w-full px-5 md:px-8">
        ...
        >
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <h1 className="font-display text-4xl sm:text-5xl md:text-7xl font-semibold tracking-tight leading-none">
                Gallery
              </h1>
        ...
        >
          {galleryImages.map((src, index) => (
            <button key={src} type="button" onClick={() => openViewer(index)} className="group block w-full text-left">
      ...
      <AnimatePresence mode="wait">
        {selectedImage && (
          ...
          >
            <div className="absolute inset-0 bg-gradient-to-b from-black/20 via-black/10 to-black/50" onClick={closeViewer} />

            <div className="absolute top-0 left-0 right-0 z-20 flex items-center justify-between px-4 sm:px-6 md:px-8 py-4 md:py-5">
              <div className="text-white/65 text-sm tracking-wide">{imageCountLabel}</div>
              <button type="button" onClick={closeViewer} className="inline-flex items-center justify-center w-11 h-11 rounded-full bg-white/10 hover:bg-white/15 border border-white/10 transition-colors backdrop-blur-md">
            ...
            <button type="button" onClick={showPrev} className="hidden md:flex absolute left-6 top-1/2 -translate-y-1/2 z-[60] items-center justify-center w-12 h-12 rounded-full bg-white/10 hover:bg-white/15 border border-white/10 backdrop-blur-md">
              <ChevronLeft className="w-5 h-5 text-white" />
            </button>
            <button type="button" onClick={showNext} className="hidden md:flex absolute right-6 top-1/2 -translate-y-1/2 z-[60] items-center justify-center w-12 h-12 rounded-full bg-white/10 hover:bg-white/15 border border-white/10 backdrop-blur-md">
            ...
            >
              <div className="mx-auto max-w-6xl rounded-[24px] bg-white/[0.04] border border-white/[0.08] backdrop-blur-xl px-3 py-3 md:px-4 md:py-4">
                <div ref={thumbnailStripRef} className="flex gap-3 overflow-x-auto no-scrollbar scroll-smooth">
                  {galleryImages.map((src, index) => {
                    ...
                    return (
                      <button
                        ...
                        className={`relative shrink-0 overflow-hidden rounded-2xl transition-all duration-300 ${
                          isActive ? "opacity-100 ring-1 ring-white/30" : "opacity-45 hover:opacity-75"
                        }`}
                      >
                        <img
